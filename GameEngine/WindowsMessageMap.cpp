...
{
	constexpr int firstColumnWidth = 25;
	const auto iterator = map.find(msg);

	std::ostringstream oss;
	if (iterator != map.end())
	{
		oss << std::left << std::setw(firstColumnWidth) << iterator->second << std::right;
	}
	
	else
	{
		std::ostringstream pads;
		pads << "Unknown message: 0x" << std::hex << msg;
		oss << std::left << std::setw(firstColumnWidth) << pads.str() << std::right;
	}
